						...
						axios.get("/getPortfolio?username=" + username)
							.then((res) => {
								if (res.status === 200) {
									...
									/* this is not good react, calling this.state right after setting it. react does not guarantee that it will update the state
		...
		return (
			<div className="container">
				...
				<div className="howToParagraph">
					...
					<p>Add a cryptocurrency in the add bar. Search results will automatically pupulate</p>
					<p>Choose your countried currency to be displayed with the button at the top right</p>
					<p>Choose the amount you own and submit it to save your data in the database</p>
					<p>Delete the Cryptocurrency from your portfolio with the red X if you no longer want to own it</p>
				</div>
				<br></br>
			...
			<select className="dropdown" onClick={(e) => { this.callCurrencyAPI(e) }}>
				...
				<div>
					...
					<UserGraph data={this.state.graphData}></UserGraph>
				</div>
			</div>
		)
	}
